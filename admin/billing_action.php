...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('order_table', 'order_number', 'order_date', 'order_time', 'order_status');
		...
		foreach($result as $row)
		{
			...
			{
				$status = '<span class="badge badge-success">In Process</span>';
				$print = '';
			}
			else
			{
			...
			$sub_array[] = '
			<div align="center">
			<button type="button" name="view_button" class="btn btn-primary btn-circle btn-sm view_button" data-toggle="tooltip" data-placement="top" title="View bill"  data-id="'.$row["order_id"].'"><i class="fas fa-eye"></i></button>
			&nbsp;
			'.$print.'
			<button type="button" name="delete_button" class="btn btn-danger btn-circle btn-sm delete_button" data-toggle="tooltip" data-placement="top" title="Delete bill"  data-id="'.$row["order_id"].'"><i class="fas fa-times"></i></button>
			</div>
			';
			$data[] = $sub_array;
		}
	...
	if($_POST["action"] == 'Edit')
	{
		...
		SET status = :status 
		WHERE appointment_number = '".$table['order_table']."' 
		";
	...
	if($_POST["action"] == 'remove_bill')
	{
		...
		";
		$object->execute();
		echo '<div class="alert alert-success">Bill deleted</div>';
	}
}
