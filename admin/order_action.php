...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'reset')
	{
		...
		foreach($table_result as $table)
		{
			...
			WHERE order_table = '".$table['appointment_number']."' 
			...
			if($object->row_count() > 0)
			{
				...
				{
					$html .= '
					<button type="button" name="table_button" id="table_'.$table["appointment_id"].'" class="btn btn-warning mb-4 table_button" data-toggle="tooltip" data-placement="top" title="Add product/medicine/treatment" data-index="'.$table["appointment_id"].'" data-order_id="'.$order["order_id"].'" data-appointment_number="'.$table["appointment_number"].'">'.$table["appointment_number"].'<br /></button>
					';
				}
			}
			else
			{
				$html .= '
				<button type="button" name="table_button" id="table_'.$table["appointment_id"].'" class="btn btn-secondary mb-4 table_button" data-toggle="tooltip" data-placement="top" title="Add product/medicine/treatment" data-index="'.$table["appointment_id"].'" data-order_id="0" data-appointment_number="'.$table["appointment_number"].'">'.$table["appointment_number"].'<br /></button>
				';
			}
		}
		echo $html;
	...
	if($_POST["action"] == 'Add')
	{
		...
		else
		{
			$order_data = array(
				...
				':order_table'			=>	$_POST['hidden_appointment_number'],
			...
			(order_number, order_table, order_gross_amount, order_tax_amount, order_net_amount, order_date, order_time, order_status) 
			VALUES (:order_number, :order_table, :order_gross_amount, :order_tax_amount, :order_net_amount, :order_date, :order_time, :order_status)
			";
	...
	if($_POST["action"] == "fetch_order")
	{
		...
		foreach($result as $row)
		{
			...
			<tr>
				...
				<td><button type="button" name="remove" class="btn btn-danger btn-sm remove_item" data-toggle="tooltip" data-placement="top" title="Remove product/medicine/treatment data-item_id="'.$row["order_item_id"].'" data-order_id="'.$row["order_id"].'"><i class="fas fa-minus-square"></i></button></td>
			</tr>
			';
		}
		$html .= '
